package com.rsh.jtoolkit.scratch.lambda;
 ...
 *
 * <p>Examples of how to construct stream pipelines dynamically using predicates and
 * context-sensitive filtering criteria. Instances of functional interfaces are stored in data
 * structures and are composed using reduce() and are then applied to the stream. This allows an
 * arbitrary number of predicates or filtering criteria to be applied in one pipeline stage.
 *
 * @author smarks
 */
public class DynamicFiltering {
  ...
  /**
   * Composes several predicates into a single predicate, and then applies the composite predicate
   * to a stream.
   */
  void composePredicates() {
    ...
    List<Predicate<Widget>> allPredicates =
        Arrays.asList(
            w -> w.length() >= 10, w -> w.weight() > 40.0, w -> w.name().compareTo("c") > 0);

    Predicate<Widget> compositePredicate = allPredicates.stream().reduce(w -> true, Predicate::and);

    widgetList.stream().filter(compositePredicate).forEach(System.out::println);
  }

  /**
   * Composes several criteria into a single criterion, and then applies the composite criterion to
   * a stream.
   */
  void composeCriteria() {
    ...
    Criterion compositeCriteria =
        allCriteria.stream().reduce(c -> c, (c1, c2) -> (s -> c2.apply(c1.apply(s))));
  ...
  /**
   * Creates a Criterion that sorts the stream using a Comparator {@code cmp} and then returns a
   * stream of the first {@code n} elements of the result.
   */
  ...
  /**
   * Creates a Criterion that sorts the stream using a Comparator {@code cmp} and then returns a
   * stream of the first {@code pct} portion of the elements. The {@code pct} value should be
   * between 0.0 and 1.0.
   */
  Criterion topPercent(Comparator<Widget> cmp, double pct) {
    return stream -> {
      List<Widget> temp = stream.sorted(cmp).collect(toList());
      return temp.stream().limit((long) (temp.size() * pct));
    };
  }

  /**
   * Creates a Criterion that sorts the stream using a Comparator {@code cmp} and then returns a
   * stream of the percentile range from {@code from} to {@code to} expressed as fractions between
   * 0.0 and 1.0.
   *
   * <p>See this followup question:
